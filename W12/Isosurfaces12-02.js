function Isosurfaces( volume, isovalue )
{
    ...
    function interpolated_vertex( v0, v1, s )
    {
        ...
        if( s0 < s1 ){
            var tmp = Math.abs( s - s0 )/Math.abs( s1 - s0 );
            var x = tmp * ( v1.x - v0.x ) + v0.x;
            var y = tmp * ( v1.y - v0.y ) + v0.y;
            var z = tmp * ( v1.z - v0.z ) + v0.z;
        }
        else{
            var tmp = Math.abs( s - s1 ) / Math.abs( s0 - s1 );
            var x = tmp * ( v0.x - v1.x ) + v1.x;
            var y = tmp * ( v0.y - v1.y ) + v1.y;
            var z = tmp * ( v0.z - v1.z ) + v1.z;
        }
